export default class HeroSection {
    constructor(props = {}) {
        const { title = 'Default Title', images = [], content = 'Default content', events = {} } = props;
    ...
    createImageContainer(images) {
        ...
        if (Array.isArray(images)) {
            images.forEach(({ src, alt }) => {
                const img = this.createElement('img', '', '', { src, alt: alt || 'Image' });
    ...
    render() {
        ...
        if (content) {
            ...
            const placeholder = this.createElement('p', 'hero-content-placeholder', 'No content available');
